...
//setup arrays of notes
int melNotes[0];
int myDurs[0];
float myVelocities[0];
"abc/OdeToJoy1.txt" => abcfile;

myReader.readFile(melNotes, myDurs, myVelocities, abcfile);

int harNotes[0];
int harDurs[0];
float harVelocities[0];
"abc/OdeToJoy2.txt" => abcfile;
myReader.readFile(harNotes, harDurs, harVelocities, abcfile);

int percNotes[0];
int percDurs[0];
float percVelocities[0];
"abc/OdeToJoy3.txt" => abcfile;
myReader.readFile(percNotes, percDurs, percVelocities, abcfile);
...
spork ~ Player3(percNotes, percDurs, percVelocities, viol3, start);
0.5 * second => now;
start.broadcast();
...
function void Player2(int N[], int D[], float V[], Bowed generator, Event start){
    while( true){
        0 => generator.gain ;
        start => now;
        for (1 => int i; i <= 4; i++) {// four repeats
...
function void Player3(int N[], int D[], float V[], ModalBar generator, Event start){
    while( true){
        0 => generator.gain ;
        start => now;
        for (1 => int i; i <= 4; i++) {// four repeats

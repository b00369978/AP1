...
// fixed !f! dynamics. %8 bar counts and B3/2 dotted rhythms
    ...
    while(str.length() > 0){
        ...
        if (char0 == "/"){// number note length
            if((char1.charAt(0) >= 48) && (char1.charAt(0) <= 57)){
                 D[ D.cap() - 1]/ (char1.charAt(0) - 48) $ int => D[ D.cap() - 1];       
        ...
        if (char0 == " "){// space
          ...
          if (char0 == "%"){// ignore %n bar count on line
            readString(); // used two characters
            continue;
            //not implemented 
        } 
        if (char0 == "!"){// ignore !f! dynamics
            do{
                readString(); // used two characters
            } until (char0 == "!");
            continue;
